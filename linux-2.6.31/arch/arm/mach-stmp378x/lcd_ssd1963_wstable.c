...
static irqreturn_t mpulcd_pxp_irq(int irq, void *dev_id)
{
...
static int mpulcd_init_pxp(dma_addr_t fb_phys)
{
    ...
    ret = request_irq(IRQ_PXP, mpulcd_pxp_irq, 0, LCD_NAME, pxp);
    if (ret) {
        printk("pxp: interrupt register failed\n");
        goto out1;
    }
