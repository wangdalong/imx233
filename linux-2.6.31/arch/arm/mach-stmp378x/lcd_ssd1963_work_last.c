...
#define PXP_WAITCOND (pxp_read(HW_PXP_NEXT) & BM_PXP_NEXT_ENABLED)
...
static void mpulcd_irq_work(struct work_struct *work)
{
//    mutex_lock(&pxp->lock);

    if (pxp->lcd_off) {
        complete(&pxp->work_off);
        return;
    }

    while (!(pxp_read(HW_PXP_STAT) & BM_PXP_STAT_IRQ))
        ;

    pxp_write(HW_PXP_STAT_CLR, BM_PXP_STAT_IRQ);
...
void mpulcd_start_refresh(void)
{
    ...
    schedule_delayed_work(&pxp->work, HZ/100);
}
...
static int mpulcd_init_pxp(dma_addr_t fb_phys)
{
    ...
    INIT_DELAYED_WORK(&pxp->work, mpulcd_irq_work);
//    mutex_init(&pxp->lock);
    init_completion(&pxp->work_off);
...
{
    pxp->lcd_off = 1;
    wait_for_completion(&pxp->work_off);
